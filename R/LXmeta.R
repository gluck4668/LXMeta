...
#------------bio packages--------------------------------
  inst_packs <- installed.packages()

  if (!requireNamespace("BiocManager", quietly = TRUE))
    install.packages("BiocManager")
  library(BiocManager)

  bio_packs <- c("impute", "pcaMethods", "globaltest", "GlobalAncova", "Rgraphviz",
                 "preprocessCore", "genefilter", "sva", "limma", "KEGGgraph",
                 "siggenes","BiocParallel", "MSnbase", "multtest", "RBGL", "edgeR",
                 "fgsea", "crmn","mzR")
  not_inst_bio <- bio_packs[!bio_packs %in% inst_packs[,1]]

  if (length(not_inst_bio)>0){
    inst_bio_fun <- function(x){BiocManager::install(x,update = F,ask = F)}
    sapply(not_inst_bio,inst_bio_fun,simplify = T)
     }


#-----common packages-------------------------------------

 com_packs <- c("devtools", "httr", "openxlsx", "magrittr", "dplyr",
                 "psych", "ggplot2", "ggrepel", "RColorBrewer", "ggthemes",
                 "tidyverse","roxygen2", "XML", "RCurl", "curl", "stringr",
                 "plyr", "patchwork","ggpubr","scales","ggplotify","purrr",
                "dendextend","VGAM")

 not_inst_com <- com_packs[!com_packs %in% inst_packs[,1]]

 if(length(not_inst_com)>0){
   not_com_fun <- function(x){install.packages(x,quiet = T,)}
   sapply(not_inst_com,not_com_fun,simplify = T)
   }
...
#-----MetaboAnalystR-------------------------------------
  ...
  install_github("xia-lab/MetaboAnalystR", force=T,build = TRUE, build_vignettes = TRUE, build_manual =T)

#------library packages-----------------------------------

 lib_packs <- c(bio_packs,com_packs,"MetaboAnalystR")
 lib_fun <- function(x){library(x,character.only = T)}
 sapply(lib_packs,lib_fun,simplify = T)
...
#------------------------------------------------------------------------------
  ...
  colnames(path_result) <- c("path_id","Total","Expected","Hits","p value","-log10(p)","Holm p","FDR","Impact")
    ...
    dw$path_id <- substring(dw[,1],1,8)

    dw$path_name <- substring(dw[,1], 9, str_length(dw[,1]))  %>% trimws()
  ...
  path_df <- left_join(path_result,path_ls,by="path_id") %>% na.omit()
  ...
  meta_path <- ggplot(path_data,
                      aes(x=Impact,
                          y=fct_reorder(path_name,Impact)))+
